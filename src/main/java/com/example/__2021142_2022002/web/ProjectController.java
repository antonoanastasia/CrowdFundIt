...
public class ProjectController {
    ...
    public String searchProjects(
        ...
        // Αν δεν δόθηκε τίποτα, δείξε όλα
        if ((keyword == null || keyword.isBlank()) && (status == null || status.isBlank())) {
            model.addAttribute("projects", service.getAllProjects()); // ή service.publicList() αν έτσι θέλεις
        } else {
            model.addAttribute("projects", service.searchProjects(keyword, status));
        }
        return "projectList"; // βεβαιώσου ότι υπάρχει projectList.html
    }
